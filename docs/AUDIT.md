# AuthFlow — Architecture & Production-Readiness Audit

A senior-level review of the codebase as it stood after the full spec was
implemented (through commit `bd9e8df`): file/directory structure, system
design, performance, memory/resource handling, dependency hygiene, and test
coverage — followed by fixes for everything that was a genuine bug or a
material production-readiness gap. Four commits came out of this,
`1cb9ecb` → `db38433`. This document is the write-up: what was found, what
was actually fixed (with evidence, not just claims), and what's deliberately
left as a documented recommendation rather than implemented.

## How this was done

Read every file under `backend/app` and `backend/tests` fresh (33 app
files, 16 test files), cross-checked every declared dependency against
actual usage via grep, and looked closely at the concurrency-sensitive
paths (password hashing, DB session handling, JWT issuance/validation)
since that's where async-framework bugs like to hide. Findings that looked
real were verified two ways before being called "fixed": the automated
suite, and a live run against the actual Neon database — not just the
isolated test DB.

## Fixed

### 1. Password hashing was blocking the entire event loop (performance, `1cb9ecb`)

`get_password_hash`/`verify_password` were plain synchronous functions
doing Argon2 work — deliberately expensive by design, that's what makes it
resistant to brute-forcing — called directly from `async def` service
functions (register, login, both profile-update paths, admin create).
Calling blocking CPU-bound code inline in an async function blocks
FastAPI's single-threaded event loop for the call's entire duration,
which stalls *every other in-flight request on that worker*, not just the
one doing the hashing.

Measured with a throwaway benchmark before fixing it: 5 concurrent
password hashes delayed an unrelated concurrent coroutine (standing in for
something like a `/health` check or another user's login) by **428ms
worst-case**. After wrapping the calls in `asyncio.to_thread`: **22ms** —
roughly a 20x reduction, and the gap widens further under real concurrent
auth traffic, since the blocking version serializes completely.

Fix: both functions are now `async`, running the actual Argon2 call via
`asyncio.to_thread`. All 5 call sites and the unit tests updated to await
them.

### 2. Password changes didn't invalidate existing JWTs (security, `037714e`)

A JWT only ever encoded `sub` (user id) and `exp`. `get_current_user`
re-checked role and soft-delete status from the DB on every request (a
deliberate design choice from earlier in the build), but never checked
whether the password had changed since the token was issued. A stolen
token stayed fully valid for its entire remaining lifetime even after the
legitimate user "secured" their account by changing the password.

Fix: added a `token_version` column, bumped on every password change
(self-service and admin-forced alike). `create_access_token` now embeds it
as a `ver` claim (plus `iat`, which was also missing); `get_current_user`
rejects any token whose version doesn't match the user's current one.

Live-verified against Neon: registered a user, logged in, confirmed the
token worked, changed the password, confirmed the *same, still-unexpired*
token was immediately rejected with 401, and a fresh login worked. Also
confirmed a *non*-password update does **not** invalidate the token —
only password changes do.

Same migration also added an index on `users.type`, since `GET /users`
filters by it and a full table scan gets real at scale. The
autogenerated migration originally added `token_version` as `NOT NULL`
with no default, which would fail against any table that already has
rows — fixed by hand with `server_default='1'` before applying it.

### 3. Inconsistent/leaky error handling, silent health check, unbounded pool (`a71d43a`)

Three related gaps, all under "will this survive contact with production":

- **No catch-all exception handler.** A genuine bug (as opposed to one of
  our own `AppError` subclasses) fell through to Starlette's default
  handler, which returns **plain text**, not JSON — every other error
  response in the app is `{"detail": "..."}`. Added a handler for the base
  `Exception` class; FastAPI/Starlette still route `AppError`/
  `HTTPException`/`RequestValidationError` to their own more specific
  handlers first (confirmed this doesn't shadow them), so this only
  catches genuinely unexpected exceptions. Logs the real traceback
  server-side, returns the generic shape to the client — no internals
  leaked.

- **`/health` was a hardcoded `{"status": "ok"}`,** unconditionally. A
  completely unreachable database would still report healthy to whatever
  is routing traffic based on that endpoint. Now does a real `SELECT 1` —
  through the same `get_db` dependency every other route uses, not the
  `engine` object directly, so it still respects the test-DB override
  instead of needing live Neon connectivity during test runs (this was a
  real mistake in the first draft of this fix, caught before committing).

- **Connection pool size/overflow/recycle were hardcoded** to whatever
  SQLAlchemy defaults to, with no way to tune them per environment. Now
  configurable via `DB_POOL_SIZE`/`DB_MAX_OVERFLOW`/`DB_POOL_RECYCLE_SECONDS`
  (Postgres only — SQLite, the zero-config local fallback never used in
  production, uses a different pool class that doesn't accept these kwargs
  at all, so it's conditional on the same `get_backend_name()` check
  already used for the SSL config).

- **`SECRET_KEY` defaulted to `"change-me"`** with nothing stopping that
  from reaching a real deployment — the classic footgun where an
  `.env.example` gets copied as-is. `Settings` now refuses to start if
  `ENVIRONMENT != "development"` and `SECRET_KEY` is still the default.

- Added a `lifespan` handler that calls `engine.dispose()` on shutdown, so
  pooled connections close cleanly instead of being dropped by process
  exit — mainly matters for graceful restarts (reload, container
  recycling).

### 4. Dead code, unused dependencies, and a coverage-measurement bug (`db38433`)

- Removed three dead stub files (`crud/base.py`, `schemas/common.py`,
  `db/init_db.py`) — all leftovers from the original scaffold's guesses at
  patterns (a generic `CRUDBase`, a generic `PagedResponse[T]`) that never
  got used because the concrete, per-feature implementations turned out to
  be simpler and clearer for a single-model app.
- Removed `python-multipart` (declared for `Form()`/`OAuth2PasswordRequestForm`-style
  parsing that was never adopted — login is JSON) and `faker` (declared,
  never imported in any test) from the dependency manifests. Also removed
  the `REFRESH_TOKEN_EXPIRE_MINUTES` setting — no refresh-token feature was
  ever built, so it was dead configuration.
- `pytest-cov` was installed but never actually invoked anywhere. Wired it
  up properly (coverage config in `pyproject.toml`, documented command in
  the README) rather than leaving it dormant.
- Doing that surfaced a genuine, separate bug: **coverage.py's default
  settings silently under-report everything that runs after an
  `await db.execute(...)`-style call**, because SQLAlchemy's async engine
  bridges to sync DB-API calls via `greenlet` switches internally, and
  coverage.py's line tracer doesn't follow those switches unless told to.
  Before the fix, `app/api/deps.py` measured 69% — with the `token_version`
  check and the soft-delete check both showing as "uncovered" *despite
  dedicated tests exercising exactly those branches*. Fixed with
  `concurrency = ["greenlet", "thread"]`. After the fix: `deps.py` is
  genuinely 100%, and the true project-wide number is **96%**, not the
  ~86% the unfixed config was reporting. The corrected numbers also
  exposed one real, still-open gap, which got a new test: a token for a
  user id that no longer exists at all (distinct from soft-deleted, which
  finds a row and was already covered).

## Current state

- **95 tests passing**, 96% line coverage (measured correctly — see above).
- Full live verification re-run against the real Neon database after all
  four fixes landed together, in one pass: register → login → get profile
  → non-password update (token still valid) → password change (old token
  now rejected) → public stats — all correct. Test data cleaned up
  afterward, `users` table empty.
- Zero known unused dependencies, zero known dead files.

## Recommended, not implemented

(Status as of the initial audit. All seven items originally listed here
were subsequently built — see "Recommendations implemented" below —
each a legitimate next step for a system that needs to handle real
production traffic and real attackers, not a fix to something that
existed, and none were asked for at the time; the gap was a documented
decision, not an oversight. Nothing currently outstanding.)

## Recommendations implemented

All seven items above, plus one gap found afterward that wasn't on the
original list, have now been built, each live-verified and committed
separately.

### Rate limiting / brute-force protection on `/auth/login`

In-process, sliding-window, keyed by client IP (`app/core/rate_limit.py`)
— explicitly documented as single-instance only; a multi-instance
deployment needs a shared store (Redis) instead, since this state isn't
shared across workers or survives a restart. Counts every request to the
endpoint regardless of outcome (simpler than tracking failures separately,
still caps credential-stuffing volume). Default: 5 attempts/60s per IP,
configurable via `LOGIN_RATE_LIMIT_MAX_ATTEMPTS`/`_WINDOW_SECONDS`. Tracked
keys are LRU-capped at 10,000 so an attacker rotating through many spoofed
IPs can't grow the limiter's own memory unboundedly — a rate limiter that
itself leaks would be a bad trade.

Live-verified against Neon: 5 wrong-password attempts each got 401: the
6th got 429, including a subsequent attempt with the *correct* password
(also blocked, since the endpoint is what's rate-limited, not just failed
credentials).

### Trigram (`pg_trgm`) indexes for the partial-match filters

`CREATE EXTENSION pg_trgm` plus a GIN `gin_trgm_ops` index on each of
`first_name`/`last_name`/`email`/`city` — the columns `GET /users` filters
with `ILIKE '%...%'`, which a plain btree index can't accelerate at all
(a leading wildcard rules that out). Declared as real `Index(...)` objects
in the `User` model, not just raw migration SQL, so a future
`alembic revision --autogenerate` recognizes them as intentional instead of
proposing to drop them. `postgresql_using`/`postgresql_ops` are
Postgres-only `Index` arguments — confirmed the full test suite still
passes on the SQLite fallback, where they're silently ignored and just
produce a harmless plain index.

Live-verified against Neon: `pg_trgm` installed, all four indexes present.
With the current near-empty table the planner correctly picks a sequential
scan (expected — cost-based, and exactly why this was filed as "matters at
scale, not today"); forcing `enable_seqscan = OFF` confirmed Postgres *can*
and *does* use `ix_users_first_name_trgm` via a Bitmap Index Scan for an
`ILIKE '%...%'` query, so the index is genuinely functional, not just
present. (Caught and fixed a related slip while checking this: that
`SET enable_seqscan = OFF` runs on a pooled connection and isn't
session-scoped-and-discarded automatically — disposed the pool afterward
so no connection with that lingering state could be handed to a real
request later.)

### Refresh token rotation

Login now returns `{access_token, refresh_token, token_type}` instead of
just an access token. `POST /auth/refresh` exchanges a refresh token for a
brand new access+refresh pair and immediately invalidates the one just
used (rotation) — a new `current_refresh_token_id` column on `User` holds
the one refresh token currently valid for that user, checked against the
incoming token's `jti` claim. Both token types now carry a `"type"` claim
(`"access"` / `"refresh"`, `create_access_token` didn't have one before)
so a refresh token can never be used to authenticate a normal request, and
vice versa — `get_current_user` and `refresh_tokens()` each check the type
they require.

Presenting an already-rotated-out refresh token (a replay of a stolen one,
most likely, though an outright forgery hits the same check) doesn't just
fail that one request — it bumps `token_version` and clears
`current_refresh_token_id`, killing *every* session for the account,
including the one legitimately created by the rotation that superseded it.
Both access and refresh tokens are already covered by the existing
password-change invalidation (`token_version`), so a compromised refresh
token dies the moment the password changes too.

Live-verified against Neon in one continuous flow: login returned both
tokens → refresh produced a working new access token → replaying the
original (now-superseded) refresh token got 401 → and killed the
legitimately-rotated session's access token too, confirming the
reuse-detection response is account-wide, not per-token.

### Admin self-protection (found live-testing, not in the original audit)

Not one of the seven audited items, but surfaced by hand-testing
immediately after: nothing stopped an admin from editing or soft-deleting
*their own* account through the admin management endpoints
(`PUT`/`DELETE /users/{id}`). Soft-deleting yourself risks locking
yourself out with no way back in — worse if you're the only admin left —
and editing yourself through the "manage everyone" route (which can also
change roles) risks an accidental self-demotion that `PUT /users/me`
already structurally prevents. Both routes now reject `user_id == the
caller's own id` with 403 before doing anything else; self-service edits
still go through `PUT /users/me` as before. `GET /users` still lists the
admin among everyone else — only editing/deleting *themselves* through
these two routes is blocked.

Live-verified against Neon: an admin appears in their own `GET /users`
listing (expected), a self-edit attempt gets 403 with account left
untouched, a self-delete attempt gets 403 with the account still active
and able to authenticate afterward.

### Structured/observability logging

Every log line is now a JSON object (`timestamp`, `level`, `logger`,
`message`, `request_id`, plus `exception` when there is one) instead of
plain text. A new `RequestIDMiddleware` assigns a UUID to each request (or
reuses one supplied via an incoming `X-Request-ID` header, so an ID
survives a hop through an upstream proxy/gateway), stores it in a
`contextvars.ContextVar`, and echoes it back on the response so a client
or support ticket can hand back the exact ID to search logs for.

The correlation isn't limited to application code: a single handler on
the *root* logger, combined with a logging filter that reads the
context var, means any logger in the process picks up the same
request_id automatically just by propagating to root — including
`sqlalchemy.engine.Engine`, so a specific SQL query line and the endpoint
that issued it show up under the same ID without the DB layer knowing
anything about HTTP requests.

That did surface one real bug while live-verifying: both uvicorn and
SQLAlchemy (`echo=True`) attach their own plain-text handler directly to
their logger the first time they're used, which happens at import time,
before app startup configures logging. Left alone, every DB query line
was logged *twice* — once structured through root, once in SQLAlchemy's
own plain-text format with no request_id attached. Fixed by stripping
each library's self-attached handler during `configure_logging()` so
everything funnels through the one handler on root.

Live-verified against Neon: `/health` returns a generated `X-Request-ID`
when the caller doesn't supply one, echoes back a caller-supplied one
unchanged, and the server's JSON log stream showed the real Postgres
handshake (`select pg_catalog.version()`, etc.) and every subsequent
query for that request tagged with the same request_id as the
`app.request` line that logged the response status and duration — with
no duplicate plain-text lines remaining.

One real gap this doesn't close: `uvicorn.access` lines log with
`request_id: null`, because uvicorn's access logging happens at the ASGI
server layer, outside the app's own middleware stack, before this
process's context var is reachable. Not fixed — would mean patching
uvicorn's own logging rather than adding application code, and the
`app.request` line already carries the same information (method, path,
status, duration) with a correct request_id.

### Security headers middleware

A new `SecurityHeadersMiddleware` adds `Strict-Transport-Security`,
`X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`,
`Referrer-Policy: strict-origin-when-cross-origin`, a restrictive
`Permissions-Policy`, and `X-XSS-Protection: 0` (explicitly disabling the
legacy filter, which is itself a known source of vulnerabilities — not
enabling it) to every response, success or error.

Deliberately no `Content-Security-Policy`: FastAPI's built-in `/docs` and
`/redoc` pages load their JS/CSS from a CDN, and a strict CSP would break
them without a much larger nonce/allowlist effort that isn't worth it for
an API that serves no other HTML. A documented gap, not an oversight.

Live-verified against Neon: `/health` carries all six headers, `/docs`
still renders (nothing broken by adding the middleware), and an
unauthenticated request to `/api/v1/users/me` (a 401) still carries the
headers too — hardening applies to error responses, not just successful
ones.

### CI running `ruff`/`black`/`mypy`/`pytest`

A GitHub Actions workflow (`.github/workflows/backend-ci.yml`) now runs
on every push/PR touching `backend/**`: ruff, black --check, mypy, then
pytest with coverage. Test DB is the same isolated in-memory SQLite the
local suite already uses (see `tests/conftest.py`), so CI needs no
database service or secrets at all.

Wiring this up surfaced a real gap first: `ruff check .` / `black
--check .` had only ever been run against individual touched files in a
given change, never the whole tree. Run for real, that was 51 lint
violations and 41 unformatted files — turning on CI against that tree
as-is would have been red from the first commit. Fixed as its own
prerequisite commit: black reformatted everything (whitespace/quoting
only), ruff's auto-fixer handled import sorting and an unused import,
one line was wrapped by hand, and `alembic/` was excluded from both
tools entirely (autogenerated migration code doesn't match black/ruff
conventions, and every future `alembic revision --autogenerate` would
violate them again regardless). Two rules are now also ignored
project-wide: `B008` (`Depends()`/`Query()` in argument defaults is
FastAPI's own dependency-injection idiom, not the mutable-default bug
the rule exists to catch — flagged on nearly every route in the
codebase) and `UP042` (`str`+`Enum` vs. `enum.StrEnum` — deliberate,
not worth the churn given how many places compare/serialize
`UserRole`).

mypy runs but is **non-blocking** (`continue-on-error: true`): 10
pre-existing type errors already existed before this workflow was
added — real gaps (`app/crud/user.py`'s SQLAlchemy filter-list typing,
and every route across `app/api/v1/endpoints/` and `app/main.py` that
returns an ORM `User` where the signature says `UserRead`, which mypy
can't see resolved by FastAPI's `response_model` at runtime). Fixing
those is a genuine cleanup, not a CI-wiring task, and making mypy
blocking today would leave the build red for debt this change didn't
introduce. Left as tracked, visible debt instead: the step still runs
and reports on every push, it just doesn't fail the build yet.

Live-verified by running every step locally — ruff, black --check,
mypy, and pytest with coverage — from a clean shell exactly as the
workflow invokes them, before and after the formatting-normalization
commit, confirming the before state was genuinely red and the after
state is genuinely green (mypy aside, by design).

### Containerization

A `backend/Dockerfile` (`python:3.11-slim`, non-root user, dependency
layer cached ahead of application code) plus a root-level
`docker-compose.yml` with three services: `postgres` (official
`postgres:16-alpine`, health-checked), `migrate` (runs `alembic upgrade
head` once `postgres` is healthy), and `api` (starts once `migrate`
completes successfully). One `docker-compose up` gets a fully working
local stack with no Neon dependency at all.

Wiring up the local-Postgres compose service surfaced a real bug:
`db/session.py` forced SSL onto *any* `postgresql` URL unconditionally.
That's correct for Neon (requires it) but wrong for docker-compose's
local Postgres, which doesn't have TLS configured — asyncpg's
`ssl=<context>` means *required*, not *preferred*, so the connection
would have failed outright rather than falling back to plaintext. Fixed
with a new `DB_SSL` setting (default `True`, matching every existing
deployment target unchanged); the compose file's `migrate`/`api`
services override it to `false`. Unit-tested directly
(`tests/unit/test_db_session.py`): SSL context attached for
Postgres+`DB_SSL=True`, no connect args for Postgres+`DB_SSL=False`,
and never attached for SQLite regardless — 3 new tests, 131 passing
project-wide.

**Update**: the local development sandbox this was originally built in
has no Docker installed, so the image was never built and the compose
stack never run *by hand* there. Rather than leave that as a standing
gap, `.github/workflows/backend-ci.yml` gained a `docker` job:
GitHub-hosted runners have Docker + Compose v2 preinstalled, so CI now
runs `docker compose up --build`, polls the `api` container's own
`HEALTHCHECK` (added to the Dockerfile for exactly this — `docker compose
ps` and `depends_on: condition: service_healthy` both need it, and
without it "the process started" and "the app actually works" are
indistinguishable) until it reports healthy, curls `/health` over real
HTTP, dumps container logs either way, then tears the stack down. That's
a real `postgres` → `migrate` → `api` boot on every push, continuously —
stronger evidence than a single one-off manual run, and it now catches a
broken Dockerfile or compose file the same way any other regression
would be caught.
